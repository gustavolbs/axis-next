...
const inventory = (...sources: Array<Partial<AxisProjectSourcesResult["sources"][number]> & { path: string }>): AxisProjectSourcesResult => ({
...
it("finds workspace and package-local scripts with independent test facts", () => {
  const result = deriveProjectFacts(inventory(
    { path: "package.json", content: JSON.stringify({ packageManager: "pnpm@9", workspaces: ["apps/*"], scripts: { test: "pnpm test", "test:coverage": "pnpm coverage" } }) },
    { path: "apps/web/package.json", content: JSON.stringify({ scripts: { test: "vitest", lint: "eslint ." } }) },
  ), "2026-09-09T00:00:00.000Z");
  expect(result.digests).toHaveLength(2);
  expect(result.facts.map((fact) => [fact.key, fact.value])).toEqual(expect.arrayContaining([
...
it("reports invalid manifests instead of treating them as absent", () => {
  ...
  expect(result.diagnostics).toEqual([expect.objectContaining({ path: "package.json", status: "invalid" })]);
...
it("uses pnpm workspace patterns for package-local facts", () => {
  ...
  expect(result.facts).toEqual(expect.arrayContaining([
    ["workspace.type", "workspace-package"],
  ].map(([key, value]) => expect.objectContaining({ key, value }))));
});
