...
const withTempProject = async (run: (root: string, outside: string) => Promise<void>) => {
  const root = await NodeFSP.mkdtemp(NodePath.join(process.env.TMPDIR ?? "/tmp", "axis-project-sources-"));
  const outside = await NodeFSP.mkdtemp(NodePath.join(process.env.TMPDIR ?? "/tmp", "axis-project-sources-outside-"));
  try { await run(root, outside); } finally { await Promise.all([NodeFSP.rm(root, { recursive: true, force: true }), NodeFSP.rm(outside, { recursive: true, force: true })]); }
};
...
it("rejects a symlink that escapes the project root", async () => {
  await withTempProject(async (root, outside) => {
    ...
    expect(result.sources).toContainEqual(expect.objectContaining({ path: "AGENTS.md", status: "failed", error: "Symlink escapes the project root." }));
  });
});
...
it("reports a missing manifest as absent", async () => {
  await withTempProject(async (root) => {
    ...
    const result = await collectProjectSources(root);
    expect(result.sources).toContainEqual(expect.objectContaining({ path: "package.json", status: "absent", content: null }));
  });
});
