...
import { AxisEffectiveContext, make, type AxisEffectiveContextInput } from "./AxisEffectiveContext.ts";
import type {
  AxisLearningScope,
...
const profileByProject = new Map<string, AxisProjectProfile>([
  [
    "manifest-first",
    {
      ...
      rules: [manifestPreferenceRule, manualPreferenceRule, manifestRestrictionRule, manualRelaxingRule],
    } as never,
  ],
  [
    "manual-first",
    {
      ...
      rules: [manualPreferenceRule, manifestPreferenceRule, manualRelaxingRule, manifestRestrictionRule],
    } as never,
  ],
]);
const tokenOrderScope = makeScope("token-order");
const tokenOrderPolicies: AxisProjectProfile["tokenEfficiencyPolicies"] = [
  { providerInstanceId: ProviderInstanceId.make("codex"), model: "gpt-5", engine: TokenEfficiencyEngineId.make("deterministic"), mode: "compress" },
  { providerInstanceId: ProviderInstanceId.make("codex"), model: "gpt-5", engine: TokenEfficiencyEngineId.make("deterministic"), mode: "record" },
  { providerInstanceId: ProviderInstanceId.make("codex"), model: "gpt-4", engine: TokenEfficiencyEngineId.make("deterministic"), mode: "record" },
];
...
const snapshot = {
  ...
  versions: [
    ...
    },
    {
      ...
      change: {
        ...
        step: { id: "verify", title: "Verify", instruction: "Run focused tests.", required: true, order: 0 },
      },
      approvedBy: "owner",
      createdAt: "2026-09-05T00:00:00.000Z",
    },
    {
      ...
      evidenceIds: ["evidence_1"],
      change: { op: "set-provider-instruction", capabilityId: "capability", instruction: "Use focused tests." },
      approvedBy: "owner",
      createdAt: "2026-09-05T00:00:00.000Z",
    },
    {
  ...
  activeVersions: [],
  activeStates: [
    { scope, targetKey: "provider:codex:step:verify", versionId: "learning_1", revision: 1, updatedAt: "2026-09-05T00:00:00.000Z" },
    { scope, targetKey: "provider:codex:step:verify", versionId: "learning_workflow", revision: 1, updatedAt: "2026-09-05T00:00:00.000Z" },
    { scope, targetKey: "provider:codex:step:verify", versionId: "learning_instruction", revision: 1, updatedAt: "2026-09-05T00:00:00.000Z" },
    { scope, targetKey: "provider:codex:step:verify", versionId: "learning_tokens", revision: 1, updatedAt: "2026-09-05T00:00:00.000Z" },
  ],
  lifecycle: [],
...
const learnedTokenState = snapshot.activeStates.find((state) => state.versionId === "learning_tokens")!;
...
const instructionVersion = snapshot.versions.find((version) => version.id === "learning_instruction")!;
const instructionState = snapshot.activeStates.find((state) => state.versionId === "learning_instruction")!;
...
const catalog = {
  ...
  capabilities: [
    { id: "capability", provider, kind: "skill", name: "Skill", enabled: true, compatibleDrivers: [], createdAt: "2026-09-05T00:00:00.000Z", updatedAt: "2026-09-05T00:00:00.000Z" },
    { id: "disabled-capability", provider, kind: "skill", name: "Disabled skill", enabled: false, compatibleDrivers: [], createdAt: "2026-09-05T00:00:00.000Z", updatedAt: "2026-09-05T00:00:00.000Z" },
    { id: "other-provider-capability", provider: otherProvider, kind: "skill", name: "Other provider skill", enabled: true, compatibleDrivers: [], createdAt: "2026-09-05T00:00:00.000Z", updatedAt: "2026-09-05T00:00:00.000Z" },
    { id: "incompatible-driver-capability", provider, kind: "skill", name: "Incompatible driver skill", enabled: true, compatibleDrivers: ["claudeAgent"], createdAt: "2026-09-05T00:00:00.000Z", updatedAt: "2026-09-05T00:00:00.000Z" },
  ],
  workHubSources: [],
} as never;
...
const dependencies = Layer.mergeAll(
  Layer.succeed(AxisProjectScope, {
    resolve: (request: { provider: { environmentId: string }; scope: { project: { environmentId: string } } }) =>
      request.provider.environmentId === request.scope.project.environmentId
        ? Effect.succeed(request)
        : Effect.fail(new AxisProjectScopeResolutionError({
  ...
  Layer.succeed(AxisContextCatalogStore, { get: Effect.succeed({ revision: 1, catalog, updatedAt: "2026-09-05T00:00:00.000Z" }) } as never),
);
const layer = it.layer(Layer.merge(Layer.effect(AxisEffectiveContext, make).pipe(Layer.provide(dependencies)), dependencies));
...
layer("AxisEffectiveContext", (it) => {
  it.effect("resolves deterministically and merges persisted and active learning", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(first.rules.map((rule) => rule.id), ["lint", "policy"]);
      assert.equal(first.capabilities.length, 1);
      assert.deepEqual(first.workflow.map((step) => step.id), ["verify"]);
      assert.deepEqual(first.providerInstructions, [{ capabilityId: AxisCapabilityId.make("capability"), instruction: "Use focused tests." }]);
      assert.deepEqual(first.tokenEfficiencyPolicies, [{ providerInstanceId: ProviderInstanceId.make("codex"), model: "gpt-5", engine: "deterministic", mode: "record" }]);
    }),
  );
  ...
  it.effect("keeps duplicate token policy digests stable when input policy order is reversed", () =>
    Effect.gen(function* () {
      ...
      assert.equal(second.tokenEfficiencyPolicies.find((policy) => policy.model === "gpt-5")?.mode, "compress");
    }),
  );
  ...
  it.effect("does not include a learned token policy for another model", () =>
    Effect.gen(function* () {
      const resolver = yield* AxisEffectiveContext;
      const result = yield* resolver.resolve(withCaller({
      ...
      assert.deepEqual(result.tokenEfficiencyPolicies.map((policy) => policy.model), ["gpt-5"]);
  ...
  it.effect("rejects a stale profile revision", () =>
    Effect.gen(function* () {
      const resolver = yield* AxisEffectiveContext;
      const error = yield* Effect.flip(resolver.resolve(withCaller({ scope, provider, step: "verify", paths: [], profileRevision: 3 })));
  ...
  it.effect("uses stable manual precedence without relaxing restrictions", () =>
    Effect.gen(function* () {
      ...
      const resolve = (projectId: string, paths: ReadonlyArray<string>) =>
        resolver.resolve(withCaller({ scope: makeScope(projectId), provider, step: "verify", paths }));
      ...
      assert.equal(manifestFirst.rules.find((rule) => rule.id === "precedence")?.text, "Manual preference.");
      assert.equal(manifestFirst.rules.find((rule) => rule.id === "protected")?.origin, "manifest");
      assert.equal(manifestFirst.rules.find((rule) => rule.id === "protected")?.effect, "restriction");
      assert.equal(
  ...
  it.effect("matches descendants of a rule path with a trailing slash", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(result.rules.map((rule) => rule.id), ["policy"]);
    }),
  );
  ...
  it.effect("does not let learned rules replace or remove manual rules", () =>
    Effect.gen(function* () {
      ...
      assert.equal(result.rules.find((rule) => rule.id === "precedence")?.text, "Manual preference.");
      assert.equal(
  ...
  it.effect("rejects learned instructions for unknown, disabled, or other-provider capabilities", () =>
    Effect.gen(function* () {
      const resolver = yield* AxisEffectiveContext;
      const result = yield* resolver.resolve(withCaller({
      ...
      assert.equal(result.capabilities.some((capability) => capability.id === "incompatible-driver-capability"), false);
      assert.equal(result.conflicts.length, 3);
      assert.equal(result.conflicts.some((conflict) => conflict.includes("unknown-capability")), true);
      assert.equal(result.conflicts.some((conflict) => conflict.includes("disabled-capability")), true);
      assert.equal(result.conflicts.some((conflict) => conflict.includes("other-provider-capability")), true);
    }),
  );
