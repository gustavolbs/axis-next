...
describe("Axis RPC caller context", () => {
  ...
  it("uses the authenticated subject before any requested scope", () => {
    expect(
      resolveAxisCallerContextId(
        { subject: "company", scopes: AuthStandardClientScopes },
        contexts,
  ...
  it("keeps legacy sessions on the server-owned personal context", () => {
    ...
    expect(resolveAxisCallerContextId(session, contexts, AxisContextId.make("company"))).toBe(
      "personal",
    );
  ...
  it("lets environment administrators select only existing catalog contexts", () => {
    ...
    expect(
      resolveAxisCallerContextId(session, contexts, AxisContextId.make("missing")),
    ).toBeUndefined();
