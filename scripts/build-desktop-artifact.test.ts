...
import { AXIS_DESKTOP_APP_ID, resolveAxisDesktopIconAssets } from "./lib/axis-brand.ts";
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("brands desktop packaging as Axis and keeps the nightly channel distinguishable", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Axis");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Axis (Nightly)");
  });

  it("packages the Axis artwork on every channel", () => {
    assert.deepStrictEqual(resolveDesktopBuildIconAssets("0.0.17"), resolveAxisDesktopIconAssets());
  ...
  // Without a publish target electron-builder omits app-update.yml, and the app
  // then reports automatic updates as unavailable. An unconfigured build must
  // still point at the Axis release feed.
  it.effect("falls back to the Axis release feed when no repository is configured", () =>
    Effect.gen(function* () {
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, `<string>ABC1234567.${AXIS_DESKTOP_APP_ID}</string>`);
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, AXIS_DESKTOP_APP_ID);
